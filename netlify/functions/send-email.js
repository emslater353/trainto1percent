...
exports.handler = async (event) => {
    ...
    try {
        ...
        switch (type) {
            ...
            case 'share_report': {
                // Share report from modal — supports multiple recipients + personal message
                const recipients = Array.isArray(data.toEmails) ? data.toEmails : [data.toEmails];
                const replyTo = data.fromEmail || FROM_EMAIL;
                const senderLabel = data.fromName ? `${data.fromName} <${replyTo}>` : replyTo;
                emailPayload = {
                    from: FROM_EMAIL,
                    to: recipients,
                    reply_to: replyTo,
                    subject: `${data.fromName || 'Someone'} shared an AI Report with you — ${data.domain}`,
                    html: generateSharedReportEmail(data)
                };
                break;
            }
...
function generateSharedReportEmail(data) {
    ...
    const displayName = (fromName && fromName.trim()) ? fromName.trim() : 'Someone';
    ...
    const oppRows = opps.map((o, i) => `
        <div style="border-left: 3px solid ${accentColor}; padding: 12px 16px; margin-bottom: 12px; background: #0d0d12;">
            ${o.prediction ? `<div style="font-size: 10px; text-transform: uppercase; letter-spacing: 1.5px; color: ${accentColor}; margin-bottom: 4px;">${o.prediction}</div>` : ''}
            <div style="font-size: 15px; font-weight: 700; margin-bottom: 4px; color: #e8e8ec;">${i + 1}. ${o.title}</div>
            <div style="font-size: 13px; color: #9090a0; line-height: 1.6;">${o.description}</div>
        </div>
    `).join('');
    ...
    const riskRows = riskList.map(r => `
        <div style="margin-bottom: 10px; padding: 10px 0; border-bottom: 1px solid #1a1a24;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-weight: 600; font-size: 13px; color: #e8e8ec;">${r.prediction}</span>
                <span style="font-weight: 700; color: #e67e22; font-size: 12px;">${r.probability}%</span>
            </div>
            <div style="font-size: 12px; color: #6e7087; margin-top: 3px;">${r.impact}</div>
        </div>
    `).join('');
...
<body style="font-family: 'Courier New', monospace; background: #050508; color: #e8e8ec; padding: 40px 20px; margin: 0;">
    <div style="max-width: 650px; margin: 0 auto;">
        ...
        <div style="text-align: center; background: #0a0a14; padding: 14px; letter-spacing: 3px; font-size: 13px; font-weight: 700; color: ${accentColor};">
            AIPROOF.CLUB
        </div>

        <div style="background: #0d0d12; border: 1px solid #1a1a24; padding: 30px;">
            <h1 style="font-size: 20px; font-weight: 700; margin: 0 0 6px 0; color: #e8e8ec;">AI Opportunity + Risk Report</h1>
            <p style="color: #6e7087; font-size: 13px; margin: 0 0 24px 0;">${domain}</p>

            <!-- Sender intro box -->
            <div style="background: #050508; border-left: 3px solid ${accentColor}; padding: 14px 18px; margin-bottom: 24px;">
                <p style="margin: 0 ${message ? '0 10px 0' : ''}; font-size: 14px; line-height: 1.7; color: #e8e8ec;">
                    <strong>${displayName}</strong> ran an AI analysis on <strong>${domain}</strong> using AI Proof Club and thought you should see it.
                </p>
                ${message ? `<p style="margin: 0; font-size: 13px; line-height: 1.7; color: #b0b0c0; border-top: 1px solid #1a1a24; padding-top: 10px; font-style: italic;">"${message}"</p>` : ''}
            </div>

            ${status ? `
            <div style="background: #050508; padding: 16px; margin-bottom: 24px; border-left: 3px solid #6e7087;">
                <div style="font-size: 10px; text-transform: uppercase; letter-spacing: 1.5px; color: #6e7087; margin-bottom: 6px;">AI-Proof Status</div>
                <div style="font-size: 18px; font-weight: 700; margin-bottom: 8px; color: #e8e8ec;">${status}</div>
                ${statusSummary ? `<p style="font-size: 13px; color: #9090a0; margin: 0; line-height: 1.7;">${statusSummary}</p>` : ''}
            </div>
            ` : ''}
            ...
            <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #1a1a24;">
                <h2 style="font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #6e7087; margin: 0 0 10px 0;">Summary</h2>
                <p style="font-size: 13px; color: #9090a0; line-height: 1.7; margin: 0;">${summary}</p>
            </div>
            ` : ''}
